...
sync_repo() {
  repo forall -c bash -c "git_reset_clean" 2>&1 1>/dev/null || true
...
[[ -n "${CCACHE_CLEAR}" && "${CCACHE_CLEAR}" != "false" ]] && export ccache_clear=1
...
then
  if egrep -qv "grapheneos|lineageos" <<< ${android_version,,}
  then
...
then
  ...
  then
    ...
    git checkout main # TODO: add tags checkout functionality (x-ref with repo tags?)
    ...
    rm -rf .git/*-apply && git_reset_clean 2>&1 1>/dev/null
    git submodule foreach "git reset --hard; git clean -ffdx" 2>&1 1>/dev/null || true
    git fetch --all --force --tags --prune --prune-tags
...
devices=$(printf "${device_list,,}" | sed -e "s/[[:punct:]]\+/ /g")
...
do
  ...
  then
    [[ "${sign}" == "1" && -z "${keys_dir}" ]] && echo "Keys Dir is required if signing build" && usage
  ...
  then
    ...
    then
      ...
      [[ "$?" != "0" ]] && echo "FATAL: GrapheneOS tag verification failed" && exit 1
      cd - >/dev/null
    ...
    then
      export build_id=$(grep "(BUILD_ID)" vendor/google_devices/${device}/${device}.mk | head -n1 | cut -d, -f2 | tr -d \))
